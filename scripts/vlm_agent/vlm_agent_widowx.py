...
from cartesian_control import CartesianDiffCtrl
from utils import get_any_frame_in_robot_base_frame,    \
                  convert_virtual_eef_quats_to_real_eef
...
def process_for_gripper_act(ik_command, gripper_command, joint_pos_des):
    ...
    if gripper_command >= 0.75:
        ...
        gripper_joint_targets = torch.Tensor([10.0])
    # Close Gripper
    elif gripper_command <= 0.25:
        # Convert 1 DoF action to 6 DoF Joint Angles for Gripper
        gripper_joint_targets = torch.Tensor([0.0])
...
def run_simulator(env: gym.Env):
    ...
    ee_goals = torch.tensor([[0.1, 0.0, 0.1, 0.707, 0.0, 0.707, 0.0, 1.0],
                             [0.1, 0.0, 0.1, 0.707, 0.0, 0.707, 0.0, 1.0],
                             [0.2, 0.0, 0.1, 0.707, 0.0, 0.707, 0.0, 0.0],
                             [0.2, 0.0, 0.1, 0.707, 0.0, 0.707, 0.0, 0.0],
                            ],
                            device=sim_cfg.device)
    '''ee_mod_goals_euler, ee_mod_goals_quats_wxyz =   \
                        convert_virtual_eef_quats_to_real_eef(ee_goals[0:3, 3:7])
    print(ee_mod_goals_quats_wxyz[0:3, 0:3])
    ee_goals[0:3, 3:7] = ee_mod_goals_quats_wxyz'''
    ...
    cartesian_ctrl = CartesianDiffCtrl(
                        scene, 
                        sim_cfg,
                        robot_entity_cfg=SceneEntityCfg(
                                            "robot", 
                                            joint_names=[".*"], 
                                            body_names=["wx250_left_finger_link"],
                                                    #["gripper_center_frame"],
                                                    #["eef_rotated_frame"]
                                                    #["wx250_gripper_prop_link"],  
                                        )
                     )
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            '''robot.set_joint_position_target(joint_pos_des, 
                                            joint_ids=cartesian_ctrl.robot_entity_cfg.joint_ids)
            print(f"----------------------> joint_pos_des {joint_pos_des}, {joint_pos_des.shape}")
            '''
            ...
            if (count % 250 == 0) or (pos_err < 5e-3 and quat_err < 5e-2): #(pos_err < 0.1 and quat_err < 0.1):
                ...
                #print(f"-------------------------------------------> Joint Action {joint_action * 180 / pi}")
