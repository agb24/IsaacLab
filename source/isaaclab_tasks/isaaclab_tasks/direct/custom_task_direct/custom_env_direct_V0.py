...
from .custom_env_cfg import UR5eRG2CustomTableEnvCfg
...
class UR5eRG2CustomTableEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: UR5eRG2CustomTableEnvCfg,
        ...
        print("---------------------------> self.robot_dof_speed_scales", 
                                                    self.robot_dof_speed_scales.shape)
        print("---------------------------> self.dt", self.dt)
        print("---------------------------> self.actions", self.actions.shape) 
        print("---------------------------> self.cfg.action_scale", self.cfg.action_scale)
        '''
    ...
    def _setup_scene(self):
        ...
        '''
        print("---------------------------> IS ROBOT FIXED BASE?????", self._robot.is_fixed_base)
        print("---------------------------> IS ROBOT FIXED BASE?????", 
                                            self._robot.root_physx_view.shared_metatype.fixed_base)
        '''
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        if self.actions[0, 6] >= 0.75:
            ...
            gripper_joint_targets = torch.Tensor([0, 0, 0, 0 ])
        # Close Gripper
        elif self.actions[0, 6] <= 0.25:
            gripper_state = 0.0
            # Convert 1 DoF action to 6 DoF Joint Angles for Gripper
            gripper_joint_targets = torch.Tensor([35*pi/180, 35*pi/180,
                                                  25*pi/180, 25*pi/180])
        
        '''
        print("\n\n\n\n\n---------------------------> actions", actions.shape)
        print("---------------------------> robot_joint_targets", robot_joint_targets.shape)
        print("---------------------------> gripper_joint_targets", gripper_joint_targets.shape)
        '''
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        print("---------------------------> INIT JOINT_POS SHAPE",
              self._robot.data.default_joint_pos,
              self._robot.data.default_joint_pos.shape,)
        print("---------------------------> DEFAULT JOINT_POS SHAPE",
              self._robot.data.default_joint_pos[env_ids],
              self._robot.data.default_joint_pos[env_ids].shape,)
        print("---------------------------> DOF_LOWER_LIMITS OUTPUT & SHAPE",
              self.robot_dof_lower_limits,
              self.robot_dof_lower_limits.shape,)
        print("---------------------------> DOF_UPPER_LIMITS OUTPUT & SHAPE",
              self.robot_dof_upper_limits,
